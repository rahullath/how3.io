...
class SafetyStabilityAnalyzer:
    """
    A specialized class for analyzing safety and stability metrics
    ...
    def __init__(self, df: pd.DataFrame):
        ...
        self.df = df
        self._clean_data()
        self.safety_columns = self._find_safety_columns()
    ...
    def _find_safety_columns(self) -> Dict[str, List[str]]:
        """Find columns related to safety and stability metrics."""
        safety_columns = {
            'validators': [],
            'developers': [],
            'governance': [],
        ...
        column_patterns = {
            'validators': [
                'validators', 'Validator', 'Node', 'nodes', 'decentralization'
            ],
            'developers': [
                'Core developers', 'Code commits', 'Developer', 'GitHub', 'git',
                'development activity'
            ],
            'governance': [
                'Governance', 'Voting', 'DAO', 'participation', 'proposal'
            ],
            'security': [
            ...
            'stability': [
                'Stability', 'Uptime', 'Reliability', 'Availability',
        ...
        for metric, patterns in column_patterns.items():
            for col in self.df.columns:
                ...
                if any(pattern.lower() in col_str for pattern in patterns):
                    safety_columns[metric].append(col)
        
        # Log found columns
        for metric, cols in safety_columns.items():
    ...
    def _get_best_column(self, metric: str, prefer_latest: bool = True) -> Optional[str]:
        ...
        Args:
            metric: The metric category ('validators', 'developers', etc.)
            prefer_latest: Whether to prefer columns with 'latest' or 'current' data
            
        Returns:
            The column name or None if no suitable column found
        """
        if metric not in self.safety_columns or not self.safety_columns[metric]:
            return None
            
        columns = self.safety_columns[metric]
    ...
    def calculate_metric_score(self, row_idx: int, metric: str, sector: str) -> Tuple[Optional[float], Dict]:
        """
        Calculate a score for a specific safety metric.
        
        Args:
            row_idx: Index of the project in the dataframe
            metric: Metric to calculate ('validators', 'developers', etc.)
    ...
    def calculate_safety_score(self, row_idx: int) -> Dict:
        """
        Calculate the overall safety and stability score based on multiple metrics.
        ...
        results = {
            ...
            'explanation': "Safety & stability score based on weighted average of key security metrics"
        }
        ...
        if weighted_scores:
            ...
            if overall_score >= 80:
                category = "Excellent Safety"
            elif overall_score >= 65:
                category = "Strong Safety"
            elif overall_score >= 45:
                category = "Moderate Safety"
            elif overall_score >= 25:
                category = "Questionable Safety"
            else:
                category = "Poor Safety"
                
            results['safety_category'] = category
        
        return results
    
    def analyze_all_projects(self, output_file: Optional[str] = None) -> pd.DataFrame:
        """
        Analyze safety and stability for all projects in the dataset.
        ...
        for idx, row in self.df.iterrows():
            ...
            print(f"Analyzing safety & stability for {project_name}...")
            
            # Calculate safety score
            project_results = self.calculate_safety_score(idx)
            
            # Add to results
            results.append({
                'Project': project_name,
                'Market Sector': project_results['sector'],
                'Safety & Stability Score': project_results['overall_score'],
                'Safety Category': project_results.get('safety_category'),
        ...
        if output_file:
            ...
            columns_to_save = ['Project', 'Market Sector', 'Safety & Stability Score']
            
            # Only include Safety Category if it exists in the DataFrame
            if 'Safety Category' in results_df.columns:
                columns_to_save.append('Safety Category')
    ...
    def visualize_sector_safety(self, output_file: Optional[str] = None) -> None:
        """
        Create visualizations of safety scores by sector.
        ...
        results_df = results_df.dropna(subset=['Safety & Stability Score'])
        
        # Plot 1: Box plot of scores by sector
        plt.subplot(2, 1, 1)
        sector_data = results_df.groupby('Market Sector')['Safety & Stability Score'].agg(['mean', 'count'])
        ...
        if sectors_to_plot:
            ...
            sns.boxplot(x='Market Sector', y='Safety & Stability Score', data=plot_df, 
                        order=sectors_to_plot)
            plt.xticks(rotation=45, ha='right')
            plt.title('Safety & Stability Scores by Market Sector')
            plt.tight_layout()
        
        # Plot 2: Top 20 safest projects
        plt.subplot(2, 1, 2)
        top_projects = results_df.nlargest(20, 'Safety &def load_data(file_path: str) -> pd.DataFrame:
    """
...
def main():
    """Main function to run the safety and stability analysis."""
    parser = argparse.ArgumentParser(description='Analyze safety and stability for crypto projects')
    parser.add_argument('--data', type=str, required=True, 
                        help='Path to data file (CSV or Excel)')
    parser.add_argument('--output', type=str, default='safety_stability_results.csv',
    ...
    try:
        ...
        analyzer = SafetyStabilityAnalyzer(df)
        ...
        if args.visualize:
            analyzer.visualize_sector_safety(args.viz_output)
        
        print(f"Analysis complete. Found {len(results)} projects with safety data.")
...
class SafetyStabilityAnalyzer:
    ...
    def visualize_sector_safety(self, output_file: Optional[str] = None) -> None:
        ...
        # Plot 2: Top 20 safest projects
        plt.subplot(2, 1, 2)
        top_projects = results_df.nlargest(20, 'Safety & Stability Score')
        
        sns.barplot(x='Safety & Stability Score', y='Project', data=top_projects, 
                    hue='Market Sector', dodge=False)
        plt.title('Top 20 Safest Projects')
        plt.tight_layout()
